...
if submit:
    ...
    # [로직 2] 일반 신청 시 7일 전 체크
        ...
        if is_sat and last_sat and (t_date - last_sat).days <= 14:
            sat_warning = "\n⚠️ 주의: 토요일 연속(혹은 격주 미준수) 사용이 감지되었습니다."

        # 시트에 기록 추가
        emergency_tag = " (당일아픔)" if is_emergency else ""
        new_row = [str(t_date), name, l_type + emergency_tag, reason]
        record_sheet.append_row(new_row)
        
        # 라인 알림 발송
        msg = f"🔔 [휴가신청]{emergency_tag}\n{name}님이 {t_date}({l_type})을 신청했습니다.{sat_warning}\n사유: {reason}"
        send_line(msg)
...
# 5. 하단 기록 및 안내
st.info("💡 0.5연차나 월차는 최소 일주일 전 신청 필수입니다. 갑자기 아픈 경우엔 '연차'만 선택 가능합니다.")
st.subheader("📋 전체 기록")
